...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs") && !e.href.includes(".htaccess")) {
            ...
            // Extract only the last folder name (album name)
            let folder = href.replace(/^.*\/songs\//, "").split("/")[0];
            // Ensure folder is defined
            if (!folder || folder === "undefined" || folder.trim() === "" || href.endsWith("/songs")) {
                console.warn("Skipping invalid folder:", href);
                continue;
            }
...
async function main() {
    ...
    currentSong.addEventListener("timeupdate", ()=>{
        ...
        // move progress bar and circle according to time
    ...
    // Add event listner to next and previous button
    ...
    document.querySelector(".volume>img").addEventListener("click", e=>{
        if(e.target.src.includes("image/volume.svg")){
            ...
            document.querySelector(".volrange").getElementsByTagName("input")[0].value = 30;
        }
    })
}
